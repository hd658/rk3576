...
static int ktl_open_device(struct kbd_to_led *ktl)
{
    if(ktl->open)
    {
       int ret = input_open_device(&ktl->handle);
       if(ret < 0)
       {
                            ...
                            char *buf)
{
    ...
    sysfs_emit(buf, "The state of kbd_to_led is %d\n",ktl->open);
...
static const struct attribute_group *ktl_attr_groups[] = {
    &ktl_attr_group,
    NULL
};
                                    ...
                                    const struct input_device_id *id)
{
    ...
    ktl->dev.groups = ktl_attr_groups;
                                    ...
                                    unsigned int code, int value)
{
    ...
    if(type == EV_KEY) //过滤掉EV_SYN,确实可以用filter函数来过滤一些事件
    {
        ...
        {
           led_set_brightness(led,(brightness == LED_OFF) ? LED_ON : LED_OFF);
        }
...
static int __init kbd_to_led_init(void)
{
...
static void __exit kbd_to_led_exit(void)
{
...
module_init(kbd_to_led_init);
module_exit(kbd_to_led_exit);
