...
static const struct reg_value ov5645_global_init_setting[] = {
	...
	{ 0x5191, 0xf8 },
	{ 0x5192, 0x04 },
	{ 0x5193, 0xfd },					// Adjust exposure
	{ 0x5194, 0xa7 },					// Adjust exposure
	{ 0x5195, 0xfc },					// Adjust exposure
	{ 0x5196, 0x03 },
	{ 0x5197, 0x01 },
	{ 0x5198, 0x04 },
	{ 0x5199, 0x12 },
...
static int ov5645_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (ov5645->power_count == !on) {
		if (on) {
			...
			ret = ov5645_set_register_array(ov5645,ov5645_check_aec,ARRAY_SIZE(ov5645_check_aec));
			if (ret < 0) {
				dev_err(ov5645->dev,
					"could not set init registers\n");
				ov5645_set_power_off(ov5645);
...
static long ov5645_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch (cmd) {
		...
		case RKMODULE_SET_QUICK_STREAM:
			stream = *((u32 *)arg);
			if (stream) {
				ov5645_set_register_array(ov5645,ov5645->current_mode->data,ov5645->current_mode->data_size);
				usleep_range(10000, 15000);
...
static int ov5645_probe(struct i2c_client *client)
{
	...
	ov5645->module_index=1;
	ov5645->module_facing="front";
	ov5645->module_name="NC";
	ov5645->len_name="NC";
